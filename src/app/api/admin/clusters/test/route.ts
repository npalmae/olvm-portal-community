...
// Prueba una conexión arbitraria (nueva o editada) antes de guardar.
// Cuerpo: { baseUrl, username?, password?, token?, allowInsecure?, caCert? }
export async function POST(request: Request) {
  ...
  if (typeof body.baseUrl !== "string" || !body.baseUrl.trim()) {
    return NextResponse.json(
      { error: "baseUrl es obligatorio para probar" },
      { status: 400 },
    );
  }
